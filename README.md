...
Why don't I use the wonderful library [async][1]? Well, `async` tries to do way too much and its `async` syntax is also very ugly.
...
These examples demonstrate how to process collections sequentially or in parallel.

Simple Sequential Example:

```javascript
var a = [
  function(finished) { setTimeout(function(){finished()}, 1); }, //executes in 1 ms
  function(finished) { setTimeout(function(){finished()}, 20); }, //executes in 20 ms
  function(finished) { setTimeout(function(){finished()}, 2); } //executes in 2 ms
];

//sequential
var results = []
batch(a).sequential()
.each(function(i, fn, done) {
  fn(function(){ results.push(i) }) //executes sequentially after timeout
})
.end(function() {
  console.dir(results) //[ 0, 1, 2 ]
})
```
...
.each(function(i, item, done) {
  fn(function() { results.push(i) })
}).end(function() {
  console.dir(results) //[ 0, 2, 1 ]
})
```
